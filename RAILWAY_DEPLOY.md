...
1. Railway принудительно настроен на builder `DOCKERFILE` (см. `railway.json`) и использует `Dockerfile` из корня репозитория.
...
- Если в логах сборки есть `pip: command not found`, проверьте что сервис использует свежий `railway.json` (builder `DOCKERFILE`) и что Root Directory указывает на папку, где лежит `Dockerfile`.
