...
- Добавлен `railway.json` с принудительным builder=`DOCKERFILE` и `healthcheckPath`.
...
1. Railway автоматически использует `Dockerfile` из корня репозитория.
...
- Nixpacks для этого репозитория отключён намеренно, чтобы исключить флуктуации автогенерации Docker-стадий.
- Если в логах снова есть `pip: command not found`, это почти всегда означает, что Railway собирает не тот сервис/ветку/Root Directory. Проверьте, что используется текущий `railway.json` (`builder=DOCKERFILE`) и `Dockerfile` из корня.
